...
abstract class MatrixRowwiseProductPEBase(implicit val p: Parameters) extends PEBase {
  ...
  val map_3o = Module(new Map(dimSize, numInputs = 3, Set(2)))
...
class MatrixRowwiseProductPEDUT(implicit override val p: Parameters)
    extends MatrixRowwiseProductPEBase
    with SingleRegReaderDUT
class MatrixRowwiseProductPEShared(implicit override val p: Parameters)
    extends MatrixRowwiseProductPEBase
    with SharedPEImpl

class RowwiseProductPEArrayDUT(nPEs: Int)(implicit p: Parameters) extends MultiIOModule {
  ...
  val (peMems, peDones) =
    generateID(cmd, nPEs, { case (_, idx) => idx.U }).zipWithIndex.map {
      case (c, idx) =>
        val pe = Module(new MatrixRowwiseProductPEShared)
...
object RowwiseProduct extends ChiselUtestTester {
  val maxInt     = 128
  implicit val p = Parameters(memAWidth = 4, regAWidth = 8)
  val tests = Tests {
    test("matrix_rowwise_product") {
      ...
      val leftValues = 0 until (1 << p.memAWidth)
      val rightValues =
        (0 until (1 << (p.regAWidth - p.memAWidth)))
          .map(_ => (0 until (1 << p.memAWidth)))
          .foldLeft(Seq[Int]())((_ ++ _))
       */
      val outValues = (rightValues.grouped(1 << p.memAWidth) toSeq).transpose map { rr =>
        (leftValues zip rr).map(a => a._1 * a._2).sum
      }

      testCircuit(
        new MatrixRowwiseProductPEDUT(),
    ...
    test("shared_rowwise_product") {
      ...
      val outRows = leftMatrix.grouped(nPEs) map { lr =>
        (rightMatrix.grouped(nPEs) toSeq).transpose map { rr =>
          (lr zip rr).map(a => a._1 * a._2).sum
        } toSeq
      } toSeq

      testCircuit(
        new RowwiseProductPEArrayDUT(nPEs),
        Seq(WriteVcdAnnotation, VerilatorBackendAnnotation)
      ) { dut =>
        dut.enable.poke(true.B)
