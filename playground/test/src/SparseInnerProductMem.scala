...
abstract class MatrixSparseInnerProductMemPEBase(implicit val p: Parameters) extends PEBase {
...
class MatrixSparseInnerProductMemPEDUT(implicit override val p: Parameters)
  extends MatrixSparseInnerProductMemPEBase
    with SingleRegReaderDUT

object SparseInnerProductMem extends ChiselUtestTester {
  val r = new Random(42)
  val maxInt     = 128
  implicit val p = Parameters(memAWidth = 5, regAWidth = 10)
  val tests = Tests {
    test("sparse_matrix_inner_product_mem") {
      val nonZero  = 13
      val rightNnz = 200
      val dimSize  = 1 << p.memAWidth
      ...
      testCircuit(new MatrixSparseInnerProductMemPEDUT, Seq(WriteVcdAnnotation, VerilatorBackendAnnotation)) { dut =>
